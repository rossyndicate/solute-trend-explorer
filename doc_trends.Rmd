...
solute <- 'DOC'
out_list <- ingest_national(site_list, solute)
out_res <- out_list[[2]]
out_data <- out_list[[1]]
...
mapview(map_data,
               ...
               cex = 'n',
               label = 'domain',
               col.regions = c('blue', 'gray', 'darkred'),
...
```{r}
solute <- 'SO4_S'
out_list_so4 <- ingest_national(site_list, solute)
...
```{r}
solute <- 'pH'
out_list_ph <- ingest_national(site_list, solute)
